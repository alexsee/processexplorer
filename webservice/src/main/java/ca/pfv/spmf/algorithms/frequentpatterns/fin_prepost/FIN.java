 ...
 * <p>
 * This implementation was obtained by converting the original C++ code of FIN provided by ZHIHONG DENG, to Java.
 * <p>
...
public class FIN {
    ...
    Set<FrequentItemset> frequentItemsets = new HashSet<>();
     ...
     */
    public Set<FrequentItemset> runAlgorithm(List<List<Integer>> database, double minsup) {
     ...
     */
    void buildTree(List<List<Integer>> database) {
        ...
        // READ THE FILE
        for (List<Integer> itemset : database) {

            // for each item in the transaction
            int tLen = 0; // tLen

            for (Integer itemX : itemset) {
     ...
     * @throws IOException
     */
    void getData(List<List<Integer>> database, double minSupport) {
        numOfTrans = 0;
    ...
    private void writeItemsetsToFile(NodeListTreeNode curNode, int sameCount) {
        ...
        if (curNode.support >= minSupport) {
            ...
            for (int i = 0; i < resultLen; i++) {
                itemset.add(item[result[i]].index);
            }
            // append the support of the itemset
            itemset.setSupport(curNode.support);
        ...
        if (sameCount > 0) {
            ...
            for (long i = 1, max = 1 << sameCount; i < max; i++) {
                ...
                for (int k = 0; k < resultLen; k++) {
                    itemset.add(item[result[k]].index);
                }
                ...
                for (int j = 0; j < sameCount; j++) {
                    ...
                    if (isSet > 0) {
                        ...
                        itemset.add(item[sameItems[j]].index);
